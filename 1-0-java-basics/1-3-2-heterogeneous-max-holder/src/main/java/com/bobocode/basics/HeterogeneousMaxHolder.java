...
public class HeterogeneousMaxHolder {
    ...
    public <T> T put(Class<T> key, T value, Comparator<? super T> comparator) {
        Comparator<T> nullSafeComparator = Comparator.nullsFirst(Objects.requireNonNull(comparator));
        var max = getMax(key);
        if(nullSafeComparator.compare(value, max) > 0)  {
            map.put(key, value);
            return max;
        }
        return value;
